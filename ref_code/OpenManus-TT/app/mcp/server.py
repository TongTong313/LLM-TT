...
class MCPServer:
    ...
    def register_tool(self,
        ...
        # MCP支持的是函数工具，但是openmanus支持的是类工具，所以需要将类工具转换为函数工具，在这里就要定一个工具函数
        ...
        required_params = tool_function.get("parameters",
                                            {}).get("required", [])
        # 可以往函数里面加字段描述
    ...
    def _build_docstring(self, tool_function: dict) -> str:
        ...
        if param_props:
            docstring += "\n\nParameters:\n"
            for param_name, param_details in param_props.items():
                required_str = ("(required)" if param_name in required_params
                                else "(optional)")
    ...
    async def cleanup(self) -> None:
        ...
        if "browser" in self.tools and hasattr(self.tools["browser"],
                                               "cleanup"):
