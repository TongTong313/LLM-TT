...
class MCPAgent(ToolCallAgent):
    ...
    async def initialize(
        ...
        if self.connection_type == "sse":
            ...
            await self.mcp_clients.connect_stdio(command=command,
                                                 args=args or [])
        else:
            raise ValueError(
        ...
        # 这里设置available_tools就是mcp_clients，mcp_clients继承ToolCollection类，汇聚了全部的工具
        ...
        # mcp_clients继承ToolCollection类，里面有tool_map是一个字典，key是工具名，value是工具类
    ...
    async def _refresh_tools(self) -> Tuple[List[str], List[str]]:
        ...
        if added_tools:
            ...
            self.memory.add_message(
                Message.system_message(
                    f"New tools available: {', '.join(added_tools)}"))
        if removed_tools:
